...
def test_init_ai_force_disable_rag_skips_rag_even_when_enabled_in_config():
    """--no-rag must skip RAGThreatGenerator entirely, even with rag.enabled: true —
    used to test AI enrichment with/without RAG, and to speed up iteration (RAG
    cold-start — chromadb + embeddings — can take minutes).
    """
    async def _run():
        with patch("os.path.exists", return_value=True):
            with patch("builtins.open", mock_open(read_data=AI_CONFIG_WITH_RAG)):
                ai_service = AIService(config_path="dummy_config.yaml", force_disable_rag=True)
        ...
        with patch("threat_analysis.server.ai_service.LiteLLMProvider") as mock_provider_class, \
            ...
            assert ai_service.rag_generator is None
    asyncio.run(_run())
...
def test_enrich_with_ai_threats_missing_title_falls_back_consistently(ai_service, caplog):
    ...
    log line must show that same fallback — not the raw None from threat_json.get('title').
    """
    async def _run():
        ai_service.ai_online = True
        ai_service.provider = MagicMock()
        ...
        no_title = {"description": "Some threat with no title field", "category": "Spoofing"}
        ai_service.provider.generate_threats_batch = AsyncMock(
            return_value={"Actor 1": [no_title]}
        )
        ...
        assert len(actor.threats) == 1
        assert actor.threats[0].description.startswith("(AI) Unknown AI Threat:")
        assert "None" not in caplog.text
        assert "Added AI threat 'Unknown AI Threat' to Actor 1" in caplog.text
    asyncio.run(_run())


def test_enrich_with_ai_threats_null_title_falls_back_consistently(ai_service, caplog):
    """The actually-observed real-world case: 'title' present but null (not absent) —
    dict.get('title', default) alone does NOT catch this, only 'or default' does.
    """
    async def _run():
        ai_service.ai_online = True
        ai_service.provider = MagicMock()
        ...
        null_title = {"title": None, "description": "Some threat", "category": "Spoofing"}
        ai_service.provider.generate_threats_batch = AsyncMock(
            return_value={"Actor 1": [null_title]}
        )
...
def test_enrich_with_ai_threats_confidence_threshold_filters_low_confidence(ai_service):
    """threat_generation.confidence_threshold was previously dead config — threats
    below the threshold must be dropped before being attached to the element.
    """
    async def _run():
        ai_service.ai_online = True
        ai_service.provider = MagicMock()
        ai_service.provider.check_connection = AsyncMock(return_value=True)
        ai_service.confidence_threshold = 0.7
        ...
        assert len(actor.threats) == 1
        assert "High" in actor.threats[0].description
    asyncio.run(_run())


def test_enrich_with_ai_threats_max_threats_per_component_caps_and_keeps_highest_confidence(ai_service):
    """threat_generation.max_threats_per_component was previously dead config — when
    set, only the top-N threats by confidence are kept per component.
    """
    async def _run():
        ai_service.ai_online = True
        ai_service.provider = MagicMock()
        ai_service.provider.check_connection = AsyncMock(return_value=True)
        ai_service.max_threats_per_component = 1
...
def test_generate_rag_threats_uses_top_k_examples_default(ai_service):
    """rag.top_k_examples was previously dead config — generate_threats() must be
    called with the configured k, defaulting to 5 when the key is absent.
    """
    async def _run():
        ai_service.rag_generator = MagicMock()
        ai_service.rag_generator.generate_threats.return_value = []
        ...
        call_kwargs = ai_service.rag_generator.generate_threats.call_args.kwargs
        assert call_kwargs["k"] == 5
    asyncio.run(_run())


def test_generate_rag_threats_uses_configured_top_k_examples(ai_service):
    async def _run():
        ai_service.ai_config["rag"]["top_k_examples"] = 8
        ai_service.rag_generator = MagicMock()
        ai_service.rag_generator.generate_threats.return_value = []
...
def test_enrich_with_ai_threats_rag_failure_does_not_abort_component_enrichment(ai_service):
    """A RAG-side exception (e.g. the prompts.yaml template bug this regression-tests)
    must not abort the whole enrichment — per-component threats must still be attached.
    Previously an unhandled exception from step 1 (RAG) propagated out of
    _enrich_with_ai_threats() entirely, so steps 2-5 (including per-component
    enrichment, the main value driver) never ran.
    """
    async def _run():
        ai_service.ai_online = True
        ai_service.provider = MagicMock()
        ai_service.provider.check_connection = AsyncMock(return_value=True)
        ai_service.rag_generator = MagicMock()
        ai_service.rag_generator.generate_threats.side_effect = KeyError('\n    "name"')
        ...
        threat_payload = {"title": "SQLi", "description": "SQL injection", "category": "Information Disclosure"}
        ai_service.provider.generate_threats_batch = AsyncMock(return_value={"Actor 1": [threat_payload]})
