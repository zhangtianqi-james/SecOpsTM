...
class LiteLLMClient:
    ...
    async def generate_content(self, prompt: str, system_prompt: str, stream: Optional[bool] = None, output_format: str = "text", max_tokens: Optional[int] = None, timeout: Optional[float] = None, temperature: Optional[float] = None):
        """
        ...
        temperature overrides the provider config value when provided (0.0 = greedy —
        used by the Red/Blue debate to reduce run-to-run variance).
        """
        ...
        effective_temperature = temperature if temperature is not None else self.provider_config.get('temperature', 0.7)
        completion_params = {
            "model": self.model_name,
            "messages": messages,
